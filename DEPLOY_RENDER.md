...
1. Push các thay đổi này và tạo **New > Blueprint** từ `render.yaml` (hoặc tạo Web Service rồi dùng các lệnh build/start trong file đó).
...
4. Mở firewall SQL Server cho outbound traffic của Render hoặc dùng SQL Server có endpoint công khai an toàn. Không thể dùng `localhost`, `127.0.0.1`, hoặc tên SQL Server trong LAN từ Render.
5. Cập nhật Google OAuth authorized redirect URI thành `https://<ten-service>.onrender.com/signin-google`, và VNPay return URL thành `https://<ten-service>.onrender.com/Customer/VnPay/PaymentCallbackVnpay`.
