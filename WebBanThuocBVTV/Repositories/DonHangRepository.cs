...
namespace WebBanThuocBVTV.Repositories
{
    public class DonHangRepository : IRepository<DonhangSanpham>
    {
        ...
        public Dictionary<string, double> Statistic()
        {
            try
            {
                ...
                double percent = ((dhCurrentMonth - dhPrevMonth) / (dhPrevMonth == 0 ? 1 : dhPrevMonth)) * 100;

                double revenueCurrentMonth = (double)_contextDB.Donhangs.Where(dh => dh.MaTrangThai == "CMP"
                                                           && dh.NgayLap.Month == DateTime.Now.Month
                                                           && dh.NgayLap.Year == DateTime.Now.Year)
                                                    .Sum(dh => dh.TongTien);
                double revenuePrevMonth = (double)_contextDB.Donhangs.Where(dh => dh.MaTrangThai == "CMP"
                                                           && dh.NgayLap.Month == DateTime.Now.AddMonths(-1).Month
                                                           && dh.NgayLap.Year == DateTime.Now.Year)
                                                    .Sum(dh => dh.TongTien);

                double percentRevenue = ((revenueCurrentMonth - revenuePrevMonth) / (revenuePrevMonth == 0 ? 1 : revenuePrevMonth)) * 100;
